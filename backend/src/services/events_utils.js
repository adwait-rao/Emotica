...
export function debugNotificationTimes(eventDate, notificationSchedule) {
  ...
  notificationSchedule.forEach((scheduleType) => {
    ...
    console.log(`\n${scheduleType}:`);
    console.log(
      `  Notification Time (IST): ${dayjs(notificationTime)
        .tz(IST_TIMEZONE)
        .format()}`
    );
    console.log(`  Is in future: ${isInFuture}`);
    console.log(
      `  Time difference: ${dayjs(notificationTime).diff(
        dayjs(),
        "minute"
      )} minutes`
    );
...
export async function createEventWithMessage(
  userId,
  sessionId,
  messageData,
  eventData
  ...
  try {
        ...
        [
          {
            ...
            content: messageData.encryptedContent,
    ...
    console.log(
      `✅ Created ${notificationEntries.length} notification entries`
    );
...
function createFallbackNotification(
  ...
  if (timeUntilEvent <= 1) {
    ...
    fallbackType = "urgent";
    console.log(
      "⚡ Event is within 5 minutes - urgent notification in 1 minute"
    );
  } else if (timeUntilEvent <= 15) {
...
async function createNotificationEntries(
  ...
  try {
    ...
    if (timeUntilEvent <= 30 && notificationSchedule.length === 0) {
      console.log(
        "🚨 Event is very soon with no schedule - using adaptive scheduling"
      );
    ...
    for (const scheduleType of notificationSchedule) {
      ...
      const isValidTime =
        notificationTimeDayjs.isAfter(bufferTime) ||
        (timeUntilEvent <= 10 &&
          notificationTimeDayjs.isAfter(now.subtract(30, "second")));
      ...
      if (isValidTime) {
        ...
        console.log(
          `✅ Scheduled ${scheduleType} notification for ${notificationTimeDayjs.format()}`
        );
      } else {
        console.log(
          `⏳ Skipped ${scheduleType} - time already passed or too close (${notificationTimeDayjs.format()})`
        );
      }
    }
...
function calculateNotificationTime(eventDateTime, scheduleType) {
  try {
    ...
    console.log(
      `📅 Calculating ${scheduleType} for event: ${eventTime.format()}`
    );
    ...
    switch (scheduleType) {
      ...
      case "same_day":
        ...
        notificationTime = calculateSameDayNotification(
          eventTime,
          now,
          timeUntilEvent
        );
      ...
      case "one_hour_before":
        ...
        if (timeUntilEvent <= 60) {
          ...
          console.log(
            `⚡ Event too soon for 1 hour notice, scheduling ${bufferMinutes} minutes before`
          );
        } else {
      ...
      case "thirty_minutes_before":
        ...
        if (timeUntilEvent <= 30) {
          ...
          notificationTime = eventTime.subtract(bufferMinutes, "minute");
          console.log(
            `⚡ Event too soon for 30 min notice, scheduling ${bufferMinutes} minutes before`
          );
        } else {
      ...
      case "fifteen_minutes_before":
        ...
        if (timeUntilEvent <= 15) {
          ...
          notificationTime = eventTime.subtract(bufferMinutes, "minute");
          console.log(
            `⚡ Event too soon for 15 min notice, scheduling ${bufferMinutes} minutes before`
          );
        } else {
      ...
      case "five_minutes_before":
        ...
        if (timeUntilEvent <= 5) {
          ...
          notificationTime = eventTime.subtract(bufferMinutes, "minute");
          console.log(
            `⚡ Event too soon for 5 min notice, scheduling ${bufferMinutes} minutes before`
          );
        } else {
    ...
    if (notificationTime.isBefore(now)) {
      console.log(
        `⚠️ Calculated time ${notificationTime.format()} is in the past, adjusting...`
      );
      notificationTime = getEmergencyNotificationTime(
        eventTime,
        now,
        timeUntilEvent
      );
    ...
    console.log(
      `⏰ ${scheduleType} calculated as: ${notificationTime.format()}`
    );
    return notificationTime.toDate();
  } catch (error) {
    console.error(
      `❌ Error calculating notification time for ${scheduleType}:`,
      error
    );
    return null;
  }
...
function calculateSameDayNotification(eventTime, now, timeUntilEvent) {
  ...
  if (timeUntilEvent > 240) {
    ...
    notificationTime = eventTime.subtract(
      Math.max(1, Math.floor(timeUntilEvent * 0.5)),
      "minute"
    );
  }

  console.log(
    `🎯 Same day notification: ${timeUntilEvent} mins until event → notify ${eventTime.diff(
      notificationTime,
      "minute"
    )} mins before`
  );
...
export async function getUpcomingNotifications(limitMinutes = 5) {
  try {
    ...
    console.log(
      `🔍 Checking for notifications between ${now.format()} and ${futureLimit.format()}`
    );
    ...
    const { data, error } = await supabase
      ...
      .gte(
        "notification_time",
        now.subtract(NOTIFICATION_BUFFER_MINUTES, "minute").toISOString()
      )
    ...
    const validNotifications = (data || [])
      .filter((notification) => {
        ...
        const isValid = notificationTime.isAfter(
          now.subtract(NOTIFICATION_BUFFER_MINUTES, "minute")
        );

        if (!isValid) {
          console.log(
            `⏳ Filtering out expired notification: ${notification.id}`
          );
        }
      ...
      .map((notification) => ({
        ...
        event_summary:
          notification.events.event_summary || notification.event_summary,
    ...
    console.log(
      `📬 Found ${validNotifications.length} valid notifications to process`
    );
